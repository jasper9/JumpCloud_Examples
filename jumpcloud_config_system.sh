...
curl -iq \
  -d "{ \"allowSshPasswordAuthentication\" : [\"true\"], \
...
# config add by JG
# pull the demo shutdown script onto the node
wget https://raw.githubusercontent.com/TheJumpCloud/SystemContextAPI/master/examples/instance-shutdown-initd -O /etc/init.d/instance-shutdown
chmod 755 /etc/init.d/instance-shutdown

# pull the teardown script onto the node also
cp /depot/scripts/jumpcloud_teardown.sh /opt/jc/jumpclound_teardown.sh
chmod 755 /opt/jc/jumpclound_teardown.sh
